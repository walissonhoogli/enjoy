...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        $product = DB::table('products')->where('id', $carts[0]['product_id'])->first();
        

        $frete = $this->correiosService->calcularFrete([
            'cep' => $carts[0]['address']['postal_code'],
            'peso' => $product->weight,
            ]);
            
        $carrier_list->each(function ($carrier, $index) use ($frete) {
            if ($index === 0) {
                $carrier->correios = $frete->getData()->pac;
            } elseif ($index === 1) {
                $carrier->correios = $frete->getData()->sedex;
            }
        });
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        $correiosId = $request->input('correios_id');
        $valorCorreios = $request->input('valor_correios');
        
        if ($correiosId === '2') {
            $correios = [
                'name_correios' => 'PAC',
                'valor_correios' => $valorCorreios
            ];
        } elseif ($correiosId === '1') {
            $correios = [
                'name_correios' => 'SEDEX',
        ...
        // Atualizar os valores na tabela shipping_info
        $shipping_info->correios = $correios['name_correios'];
        $shipping_info->valor_correios = $correios['valor_correios'];
        ...
        if ($carts && count($carts) > 0) {
            foreach ($carts as $key => $cartItem) {
                ...
                if(get_setting('shipping_type') != 'carrier_wise_shipping' || $request['shipping_type_' . $product->user_id] == 'pickup_point'){
                    ...
                    $cartItem['shipping_cost'] = $correios['valor_correios'];
                }
            ...
            return view('frontend.payment_select', compact('carts', 'shipping_info', 'total', 'correios'));
