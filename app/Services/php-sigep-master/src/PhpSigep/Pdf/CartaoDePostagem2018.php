...
$numObjPostais = count($objetosPostais);
for ($i = 0; $i < $numObjPostais; $i++) {
            $this->pdf->AddPage();
     ...
     $logoChancelaPAC = realpath(dirname(__FILE__)) . '/PACCHANLEASTFNOVO.png';
     $logoChancelaSEDEX = realpath(dirname(__FILE__)) . '/CHANCELASEDEXSTF.png';
  
            $this->pdf->SetXY($lPosFourAreas, $tPosFourAreas);
            $this->setFillColor(222, 222, 222);
            if ($this->logoFile) {
                $this->pdf->Image($this->logoFile, 75, ($this->pdf->GetY() + 2), 30, 30);
            }
...
switch ($servicoDePostagem->getCodigo()) {
    case '03298':
        ...
        $_texto = 'PAC';
        $this->pdf->Image($logoChancelaPAC, 5, ($this->pdf->GetY() + 2), 25, 25);
        break;
    case '03220':
        $simbolo_de_encaminhamento = realpath(dirname(__FILE__)) . '/simbolo-sedex-standard.png';
        $_texto = 'SEDEX';
        $this->pdf->Image($logoChancelaSEDEX, 5, ($this->pdf->GetY() + 2), 25, 25);
        break;
    default:
        // código padrão a ser executado caso não haja nenhum case correspondente
        break;
}
            ...
            // } else if ($chancela) {
            //     // $chancela->draw($this->pdf($chancela, 5, ($this->pdf->GetY() + 2), 25, 25));
                       
            //    
              
            // }
            
            // if(ServicoDePostagem::SERVICE_SEDEX_CONTRATO_AGENCIA){
            //     $this->pdf->Image($logoChancelaSEDEX, 5, ($this->pdf->GetY() + 2), 25, 25);
            ...
            $this->pdf->SetFont('', 'B');
            $etiquetaComDv = $objetoPostal->getEtiqueta();
    ...
    private function writeRemetente ($l, $t, $w, \PhpSigep\Model\Remetente $remetente)
    {
        ...
        $nomeRemetente = config('correios.nome_remetente');
        $logradouro = config('correios.logradouro');
        $numero = config('correos.numero');
        $complemento = config('correios.complemento');
        $bairro = $remetente->getBairro();
        $cidade = config('correios.cidade');
        $uf = config('correios.uf');
